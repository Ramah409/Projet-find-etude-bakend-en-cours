...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http, JwtService jwt) throws Exception {
        http
            ...
            // 3) j' insère le filtre JWT AVANT le filtre standard de Spring Security
            //    → ainsi, si un header Authorization: Bearer <token> est présent, je pose l’Authentication.
            .addFilterBefore(new JwtAuthFilter(jwt), UsernamePasswordAuthenticationFilter.class)

            // 4) Règles d’autorisation : genre qui peut appeler quoi ?
            .authorizeHttpRequests(auth -> auth

                // a) Toujours autoriser les requêtes OPTIONS 
                .requestMatchers(HttpMethod.OPTIONS, "/**").permitAll()

                // b) Routes d’authentification publiques (inscription / connexion)
                //     Mon front n’a PAS de token au moment de s’inscrire/se connecter
                .requestMatchers("/api/auth/**").permitAll()

                // toutes les autres routes “métier” sous /api/** exigent d’être authentifié
